"""
Contains the class Model_metrics for the model optimization metrics.
"""
import pandas as pd
import numpy as np
...
class Model_metrics:
    """
    A class to manage the needed metrics of our predictive models.
    """
    
    def __init__(self, model, df, cutoff):
        # The argument 'df' should be the validation dataset
        self.model                     = model
        self.cutoff                    = cutoff
        
        # The error metrics are managed through a data frame
        self.error_df                  = pd.DataFrame()
        self.error_df['probability']   = model.predict_proba(df.drop(['Class'], axis=1))[:,1] # Probability of a true value (a fraud)
        self.error_df['labels']        = np.array(df.Class)
        ...
        self.n_P                            = df.Class.sum() # number of fraud on test set
        self.n_N                            = len(df) - self.n_P  # number of true transactions on test set
